...
public class AutonProcedureLHF extends SequentialCommandGroup {
    ...
    public AutonProcedureLHF(Drivetrain drivetrain, Intake intake, Indexer indexer, Shooter shooter) {
        ...
        addCommands(new ParallelDeadlineGroup(new StepTwo(drivetrain, Constants.DrivetrainConstants.Autonomous.Offsets.AUTO_LF_H_OFFSET), new BetterIntakeStop(intake), new IndexerStop(indexer))); //drive back to start point(maybe just put drivetrain falcons in brake?)
